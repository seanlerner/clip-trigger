...
function add_triggers_from_trigger_directry_and_local(err, resp, body) {
  ...
  fs.readdirSync(remote.app.CT.dir.installed_triggers).forEach(add_trigger_from_local)
...
function install_trigger(tr_id) {
  const tr  = document.getElementById(tr_id),
        git = remote.app.CT.vendor.simple_git(remote.app.CT.dir.installed_triggers),
...
function uninstall_trigger(trigger) {
  const tr           = document.getElementById(trigger),
        trigger_path = path.join(remote.app.CT.dir.installed_triggers, trigger)
