...
function add_triggers_from_trigger_directry_and_local(err, resp, body) {
  if (body)
    triggers = JSON.parse(pretend_body)
  else
...
function tr_html(id, attrs) {
  ...
  template.innerHTML =
    `<tr id=${id} data-url=${attrs.url}>
       ...
       <td>${id}</td>
       <td class=center>${attrs.trigger_code}</td>
       <td>
       ...
       <td>
         <a href="javascript:shell.openExternal('${attrs.url}')">Homepage</a>
       </td>
     </tr>`
...
pretend_body = `
{
  "bacon-ipsum-trigger": {
    "name": "Bacon Ipsum",
    "url": "",
    "description": "Get Bacon Ipsum from the internet",
    "trigger_code": "bc"
  },
  "clip-trigger-trigger": {
    "name": "Clip Trigger",
    "url": "",
    "description": "Use Clip Trigger triggers to manage Clip Trigger",
    "trigger_code": "ct"
  },
  "hello-world-trigger": {
    "name": "Hello, World!",
    "url": "https://github.com/seanlerner/hello-world-trigger",
    "description": "A simple Hello, World! trigger to test if Clip Trigger is working",
    "trigger_code": "hw"
  },
  "spotify-trigger": {
    "name": "Spotify",
